...
2. **Fetch activity via GraphQL** — Query PRs (both created and merged), issues, and commits:
    ...
    - Deduplicate those commits against the PR set before ranking: drop commits whose SHA is already reachable from a collected PR (`gh pr view <pr-url> --json commits`), and treat the remainder as direct-commit topics.
